...
impl OID4VPDraft20Service {
    pub async fn get_client_request(
        ...
        let proof = self
            ...
            .ok_or(OID4VPDraft20ServiceError::MissingProof(id))?;
        ...
        let interaction =
            proof
                .interaction
                .as_ref()
                .ok_or(OID4VPDraft20ServiceError::MappingError(
        ...
        let OpenID4VPVerifierInteractionContent {
            ...
            return Err(OID4VPDraft20ServiceError::MappingError(
        ...
        Ok(match client_id_scheme {
            ...
            ClientIdScheme::VerifierAttestation => {
                format_authorization_request_client_id_scheme_verifier_attestation(
                    ...
                    interaction_data.response_uri.ok_or(
                        OID4VPDraft20ServiceError::MappingError("response_uri missing".to_string()),
                    )?,
    ...
    pub async fn get_client_metadata(
        &self,
        id: ProofId,
    ) -> Result<OpenID4VPDraftClientMetadata, OID4VPDraft20ServiceError> {
    ...
    pub async fn direct_post(
        ...
        let proof = self
            ...
            .await
            .error_while("getting proof")?
            .ok_or(OID4VPDraft20ServiceError::MissingProofForInteraction(
                interaction_id,
            ))?;
        ...
        match request {
            ...
            OpenID4VPDirectPostRequestDTO {
                ...
                let key_id = jwe_header.key_id.as_ref().ok_or(
                    OID4VPDraft20ServiceError::ValidationError("Missing JWE key_id".to_string()),
                )?;
                ...
                let key = key
                    ...
                    .ok_or_else(|| {
                        OID4VPDraft20ServiceError::ValidationError(
                ...
                Ok(SubmissionRequestData {
                    ...
                    state: payload
                        .state
                        .ok_or(OID4VPDraft20ServiceError::ValidationError(
