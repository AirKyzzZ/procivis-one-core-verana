...
pub(crate) type DataElements = HashMap<DataElementIdentifier, IntentToRetain>;
pub(crate) type DataElementIdentifier = String;
pub(crate) type IntentToRetain = bool;
...
pub(crate) struct EDeviceKey(pub x25519_dalek::PublicKey);
...
pub(crate) struct EReaderKey(x25519_dalek::PublicKey);
...
pub(crate) struct KeyAgreement<PK> {
    pk: PK,
    sk: EphemeralSecret,
}
...
impl KeyAgreement<EDeviceKey> {
    ...
    pub(crate) fn device_key(&self) -> &EDeviceKey {
        &self.pk
    }
...
impl KeyAgreement<EReaderKey> {
    ...
    pub(crate) fn reader_key(&self) -> &EReaderKey {
        &self.pk
    }
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub(crate) struct SkDevice {
    #[serde(with = "secret_slice")]
    secret_key: SecretSlice<u8>,
}

impl SkDevice {
    pub(crate) fn new(secret_key: SecretSlice<u8>) -> Self {
        Self { secret_key }
    }
    ...
    pub(crate) fn decrypt(&self, ciphertext: &[u8]) -> anyhow::Result<Vec<u8>> {
