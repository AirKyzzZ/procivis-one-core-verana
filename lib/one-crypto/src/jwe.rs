...
pub struct Header {
    ...
    // decoded apv param
    // Optional per RFC 7518 §4.6.2 (defaults to empty)
    pub partyvinfo_data: Option<Vec<u8>>,
}
...
pub fn extract_jwe_header(jwe: &str) -> Result<Header, EncryptionError> {
    ...
    let partyuinfo_data = header
        .agreement_partyuinfo
        .map(|apu| decode_b64(apu.as_str(), "apu"))
        .transpose()?;
    let partyvinfo_data = header
        .agreement_partyvinfo
        .map(|apv| decode_b64(apv.as_str(), "apv"))
        .transpose()?;
    Ok(Header {
        partyuinfo_data,
        partyvinfo_data,
...
pub(super) fn derive_encryption_key(
    ...
    other_info.extend((apu_data.len() as u32).to_be_bytes());
    other_info.extend(apu_data);
    other_info.extend((apv_data.len() as u32).to_be_bytes());
    other_info.extend(apv_data);
