...
pub struct JweHeader<T> {
    ...
    pub agreement_partyuinfo: Option<String>,
    // apv param — optional per RFC 7518 §4.6.2
...
pub fn build_jwe(
    ...
    let apu = header.agreement_partyuinfo.as_deref().unwrap_or("");
    let apv = header.agreement_partyvinfo.as_deref().unwrap_or("");
    let apu_b64 = Base64UrlSafeNoPadding::encode_to_string(apu)
        .map_err(|e| EncryptionError::Crypto(format!("failed to encode apu: {e}")))?;
    let apv_b64 = Base64UrlSafeNoPadding::encode_to_string(apv)
...
pub fn extract_jwe_header(jwe: &str) -> Result<Header, EncryptionError> {
    ...
    let agreement_partyuinfo = header
        .agreement_partyuinfo
        .map(|apu| {
            let bytes = decode_b64(apu.as_str(), "apu")?;
            String::from_utf8(bytes)
                .map_err(|e| EncryptionError::Crypto(format!("Failed to parse apu to string: {e}")))
        })
        .transpose()?;
    let agreement_partyvinfo = header
        .agreement_partyvinfo
        .map(|apv| {
            let bytes = decode_b64(apv.as_str(), "apv")?;
            String::from_utf8(bytes)
...
mod test {
    ...
    async fn test_decrypt_jwe_ec() {
        ...
        let expected_header = Header {
            ...
            agreement_partyuinfo: Some(
                "\u{18}G\u{7}Ëcr\u{5}Þ§ù\u{97}~Ê0W>9FÖ=\u{17}¹v\u{7f}®ô\u{c}h\u{1c}u\u{99}Ç"
                    .to_string(),
            ),
            agreement_partyvinfo: Some("bueFnxmWT1EJEmPB5zq4m6aqkhEjIN8j".to_string()),
        };
    ...
    async fn test_decrypt_jwe_eddsa() {
        ...
        let expected_header = Header {
            ...
            agreement_partyuinfo: Some(
                "BJ\"ûzw\u{11}\u{93}\u{7}Ç>»Ý%\nÁk&âÌÕ°\u{6}\u{9e}Õ_\u{8a}Ö%#Za".to_string(),
            ),
            agreement_partyvinfo: Some("e4xmMaGk6O2UX25eq7Opc46LU7PdzUXp".to_string()),
        };
