...
use crate::entity::wallet_instance;
use crate::list_query_generic::SelectWithListQuery;
use crate::mapper::{to_data_layer_error, to_update_data_layer_error};

#[autometrics]
#[async_trait::async_trait]
impl WalletInstanceRepository for WalletInstanceProvider {
    async fn create_wallet_instance(
        ...
        self.db
            .tx(async {
                let wallet_unit = wallet_instance::ActiveModel::try_from(request)?
                    .insert(&self.db)
                    .await
    ...
    async fn get_wallet_instance(
        ...
        let Some(wallet_unit) = wallet_instance::Entity::find_by_id(id)
            .one(&self.db)
            .await
            .map_err(to_data_layer_error)?
        else {
        ...
        if let Some(attested_key_relations) = &relations.attested_keys {
            let attested_keys = self
                ...
                .get_by_wallet_instance_id(&wallet_unit.id, attested_key_relations)
                .await?;
    ...
    async fn get_wallet_instance_list(
        ...
        let mut query = wallet_instance::Entity::find();
        ...
        if query_params.sorting.is_some() || query_params.pagination.is_some() {
            // fallback ordering
            query = query
                .order_by_desc(wallet_instance::Column::CreatedDate)
                .order_by_desc(wallet_instance::Column::Id);
        }

        let wallet_units = query.all(&self.db).await.map_err(to_data_layer_error)?;

        let total_items = wallet_instance::Entity::find()
    ...
    async fn update_wallet_instance(
        ...
        let update_model = wallet_instance::ActiveModel {
            id: Unchanged(*id),
    ...
    async fn delete_wallet_instance(&self, id: &WalletInstanceId) -> Result<(), DataLayerError> {
        wallet_instance::Entity::delete_by_id(id)
            .exec(&self.db)
            .await
            .map_err(to_update_data_layer_error)?;
        Ok(())
