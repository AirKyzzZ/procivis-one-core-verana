...
async fn test_upsert_wallet_instance_attested_key_success() {
    ...
    let id: WalletInstanceAttestedKeyId = Uuid::new_v4().into();
    let request = WalletInstanceAttestedKeyUpsertRequest {
        id,
        wallet_instance_id: test_setup.wallet_unit_ids[0],
    ...
    let reloaded = provider
        ...
        .unwrap()
        .unwrap();
    assert_eq!(reloaded.wallet_instance_id, request.wallet_instance_id);
...
async fn test_upsert_wallet_instance_attested_key_conflict_success() {
    ...
    let original_attested_key = WalletInstanceAttestedKey {
        id,
        wallet_instance_id: test_setup.wallet_unit_ids[0],
        created_date: now,
        last_modified: now,
    ...
    let request = WalletInstanceAttestedKeyUpsertRequest {
        id,
        wallet_instance_id: test_setup.wallet_unit_ids[1],
        expiration_date: one_core::clock::now_utc(),
        public_key_jwk: random_jwk(),
    };
    provider.upsert_attested_key(request.clone()).await.unwrap();
...
async fn test_get_wallet_instance_attested_key_by_wallet_instance() {
    ...
    let id: WalletInstanceAttestedKeyId = Uuid::new_v4().into();
    let now = one_core::clock::now_utc();
    let attested_key = WalletInstanceAttestedKey {
        id,
        wallet_instance_id: test_setup.wallet_unit_ids[0],
        created_date: now,
        last_modified: now,
    ...
    let reloaded = provider
        .get_by_wallet_instance_id(&test_setup.wallet_unit_ids[0], &Default::default())
        .await
        .unwrap();
...
struct TestSetup {
    pub provider: WalletInstanceAttestedKeyProvider,
...
async fn setup(n: usize) -> TestSetup {
    ...
    TestSetup {
        provider: WalletInstanceAttestedKeyProvider {
